...
ALLOWED_TOOLS = {"nmap", "sqlmap", "nuclei", "whatweb", "whois", "dig", "nslookup", "curl", "wget", "ping", "traceroute", "nikto"}
...
def _sanitize_arg(arg: str) -> str:
    ...
    for ch in forbidden:
        if ch in arg_str:
            raise ValueError(f"Argument contains forbidden character: {repr(ch)}")
    return arg_str


def _run_tool(tool: str, args: list, timeout: int) -> dict:
    import subprocess
    tool_lower = tool.lower().split()[0]
    if tool_lower not in ALLOWED_TOOLS:
        return {"tool": tool, "error": f"Tool not allowed: {tool}", "success": False}
    try:
        safe_args = [_sanitize_arg(a) for a in args]
    except ValueError as e:
        return {"tool": tool, "error": str(e), "success": False}
    try:
        result = subprocess.run(
            [tool] + safe_args, capture_output=True, text=True, timeout=timeout,
        )
        return {
            "tool": tool,
            "args": safe_args,
...
def akatsuki_chain(action: str, target: str = "", objective: str = "Full penetration test",
    ...
    if action == "list_playbooks":
        try:
            ...
            cfg = load_config()
            pb_dir = Path(cfg.get("paths", {}).get("playbooks_dir", "playbooks"))
        except ImportError:
